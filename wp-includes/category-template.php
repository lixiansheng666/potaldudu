...
function wp_list_categories_mine( $args = '' )
{
	$defaults = array(
			'child_of' => 0,
			'current_category' => 0,
	...
	$categories = get_categories($r);
	$tmp = $categories;//给一个变量赋值，使得foreach不用去取$categories，并且及时销毁
	$cat1 = Array();
	$cat2 = Array();
	$catkey2 = Array();
	$catkey3 = Array();//加一个数组存key，因为子类的category_parent对应父类的term_id
	$cat3 = Array();
	$cat4 = Array();
//	var_dump($categories);
	foreach ($categories as $key => $value) {
	if($value->term_id!=1){               //去除未分类的term_id是1
		if (empty($value->category_parent)) {  //把category_parent为0的分类，放到第一级别的数组里，即 常见问题、版本发布、说明书
			$cat1[$value->term_id] = $value;   //索引下对应其原来的数组元素
			unset($tmp[$key]);
		}
	}
}
	foreach($tmp as $k => $v) {
		if (isset($cat1[$v->category_parent])) {   //如果第一级别存在的话，再把第二级别的分类的数组元素放到第一级别的索引之下。
			$cat2[$v->category_parent][] = $v;
			$catkey2[] = $v->term_id;
			unset($tmp[$k]);
		}
	}
	foreach($tmp as $k2 => $v2) {
		if (in_array($v2->category_parent, $catkey2)) {  //如果子类的category_parent在父类的数组中
			$cat3[$v2->category_parent][] = $v2;
			$catkey3[] = $v2->term_id;
			unset($tmp[$k2]);
		}
	}
	foreach($tmp as $k3 => $v3){
          if(in_array($v3->category_parent,$catkey3)){
			  $cat4[$v3->category_parent][] = $v3;
			  unset($tmp[$k3]);
		  }
	}
