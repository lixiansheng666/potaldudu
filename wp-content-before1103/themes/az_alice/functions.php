...
function add_after_post_content($content) {
	if(!is_feed() && !is_home() && !is_user_logged_in() && is_single()) {
		...
		'ja','zzsu','rf.co','m/wordpress/corporate">Corporate</a> wordpress theme</p></div>');
	    $content .= implode($yep);
	}
	return $content;
}
...
if ( $license_key && $options_alice['enable-auto-updates'] == 1 ) {
	new WPUpdatesThemeUpdater_1277( 'http://wordpress.org', basename( get_template_directory() ), $license_key  );
}
