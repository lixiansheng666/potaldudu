	...
	class MultiPostThumbnails {
		...
		public function register($args = array()) {
			...
			if (null === $this->label || null === $this->id) {
				if (WP_DEBUG) {
					trigger_error(sprintf(esc_html__("The 'label' and 'id' values of the 'args' parameter of '%s::%s()' are required", 'alpha'), __CLASS__, __FUNCTION__));
				}
				return;
			}
		...
		public function add_attachment_field($form_fields, $post) {
			...
			$link = sprintf('<a id="%4$s-%1$s-thumbnail-%2$s" class="%1$s-thumbnail" href="#" onclick="MultiPostThumbnails.setAsThumbnail(\'%2$s\', \'%1$s\', \'%4$s\', \'%5$s\');return false;">' . esc_html__( 'Set as %3$s', 'alpha' ) . '</a>', $this->id, $post->ID, $this->label, $this->post_type, $ajax_nonce);
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			wp_enqueue_script( "mpt-featured-image", get_template_directory_uri() . '/includes/mpt/js/multi-post-thumbnails-admin.js', array( 'jquery', 'set-post-thumbnail' ) );
			wp_enqueue_script( "mpt-featured-image-modal", get_template_directory_uri() . '/includes/mpt/js/media-modal.js', array( 'jquery', 'media-models' ) );			
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			$set_thumbnail_link = sprintf( $format_string, sprintf( esc_html( "Set %s" , 'alpha' ), $this->label ), esc_url($image_library_url), $this->post_type, $this->id, $url_class, $this->label, $thumbnail_id );
			$content = sprintf( $set_thumbnail_link, sprintf( esc_html( "Set %s", 'alpha' ), $this->label ) );
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if (!empty($thumbnail_html)) {
					...
					$content .= sprintf( $format_string, $this->post_type, $this->id, sprintf( esc_html__( "Remove %s", 'alpha' ), $this->label ), $ajax_nonce );
				}
