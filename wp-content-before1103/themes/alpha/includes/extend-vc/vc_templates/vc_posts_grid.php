...
while( $all_posts->have_posts() ) { 
    ...
    $title_permalink = ( $post_format == 'link' ) ? get_post_meta( $post_id, 'alpha_post_link', true ) : get_permalink();
    $title_string = ( $post_format == 'quote' ) ? get_post_meta( $post_id, 'alpha_post_quote_1', true ) : get_the_title(); 
    ...
    if ( $style == 'list' ) {
    	...
    	$img_size = alpha_portfolio_thumbnails_size( $style, $cols );
    }
    ...
    if ( $style == 'list' ) {
	    ...
	    $output .= '<div class="content clearfix">
			...
			<time class="time" pubdate datetime="' . get_the_time( 'c' ) . '"><span class="l">' . get_the_time( esc_html__( 'F j, Y', 'alpha' ) ) . '</span><span class="s">' . get_the_time( esc_html__( 'm/j/y', 'alpha' ) ) . '</span></time>
		</div>';
		...
		<time pubdate datetime="' . get_the_time( 'c' ) . '">' . get_the_time( esc_html__( 'F j, Y', 'alpha' ) ) . '</time>';

		$output .= '<p>' . alpha_excerpt( 'alpha_excerptlength_post' ) . '</p>';
