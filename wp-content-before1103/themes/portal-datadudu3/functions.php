...
function displayDropdownMenu($targetParentSlug,$iconRelatedURL,$displayLevel)
{
...
//                var_dump($cat->name);
        ?>
				<?php if($displayLevel != 1) { ?>
		<li style="margin: 0px;" class="<?php if (in_category($cat->cat_ID)&&is_single($page_id)) echo "open" ?>">
		<div class="link" style="padding: 5px 15px 5px 30px;">
		<p><?php echo $cat->name; ?></p><i class="fa fa-chevron-down"></i>
		</div>
		<ul class="submenu" style="margin: 0px;<?php if (in_category($cat->cat_ID)&&is_single($page_id)) echo "display: block;" ?>">
               <?php }elseif($displayLevel == 1){ ?>
		<li style="margin: 0px;" >
		<div class="link2" style="padding: 5px 15px 5px 30px;">
		<p><a href="<?php echo get_category_link($cat->cat_ID); ?>" style="text-decoration:none;color:#727272;"><?php echo $cat->name; ?></a></p>
		</div>
...
//                var_dump(get_post_permalink($post1->ID));
            if (in_category($cat->cat_ID) && $post1->ID == $page_id) {
                ?>
		<li style = "margin: 0px;background-color: #2aabd2;" ><a title = "" href = '<?php echo get_permalink($post1->ID); ?>' ><?php echo $post1->post_title; ?> </a>
		</li>
		<?php
    } else {
        ?>
		<li style = "margin: 0px;" > <a title = "" href = '<?php echo get_permalink($post1->ID); ?>' ><?php echo $post1->post_title; ?> </a>
		</li>
		<?php
    }
    ?>
...
//           var_dump($sub2);
    ...
    ?>
				<?php if (in_category($cat2->cat_ID)){ ?>
		<li style ="margin: 0px;background-color: #2aabd2;"><a title = "<?php echo $cat2->name; ?>" href = '<?php echo get_category_link($cat2->cat_ID); ?>' ><?php echo $cat2->name; ?></a></li>
				<?php }else{ ?>
