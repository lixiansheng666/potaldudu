...
function dh_enqueue_google_font(){
	...
	if(!empty($google_fonts)){
		foreach ($google_fonts as $font=>$google_font){
			wp_enqueue_style( 'dh-'.sanitize_title($font), "http://fonts.googleapis.com/css?family=$font:".implode(',', array_unique( (array) $google_font['style']))."&subset=".implode(',', array_unique((array)$google_font['subset'])),false);
		}
	}
}
...
function dh_page_title($echo = true){
	...
	elseif (is_search())
	{
		...
		if(!empty($wp_query->found_posts))
		{
			...
			{
				$title =  $wp_query->found_posts ." ". __('search results for','sitesao').' &ldquo;'.esc_attr( get_search_query() ).'&rdquo;';
			}
			else
			{
				$title =  $wp_query->found_posts ." ". __('search result for','sitesao').' &ldquo;'.esc_attr( get_search_query() ).'&rdquo;';
			}
		}
		else
		{
			if(!empty($_GET['s']))
			{
				$title = __('Search results for','sitesao').' &ldquo;'.esc_attr( get_search_query() ).'&ldquo;';
			}
			else
			{
