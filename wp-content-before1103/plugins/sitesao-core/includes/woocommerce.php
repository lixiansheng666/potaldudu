...
if ( ! class_exists( 'DH_Woocommerce' ) ) :
	class DH_Woocommerce {
		...
		public function product_reviews_tab_title($title, $key){
			global $product, $post;
			if($key === 'reviews'){
				return sprintf( __( 'Reviews <span>%d</span>', 'sitesao' ), $product->get_review_count() );
			}
			return $title;
		}
		...
		protected function _get_minicart( $content = false, $_flag = false ,$topbar=false) {
			...
			if( ((dh_get_theme_option('woo-minicart-style','side') == 'mini') ||  $content)){
				if (! empty( $cart_count ) ) {
					$minicart .= '<div class="minicart-header">' . $cart_count_text . ' ' . sprintf(__( 'in the <a href="%1$s">shopping cart</a>', 'sitesao' ),$cart_url) . '</div>';
		...
		public function the_product_video(){
			...
			if(!empty($video)){
				...
				$html .= '</div>';
				echo ($html);
				echo '<a class="video-embed-action" data-container="body" data-trigger="hover" data-toggle="tooltip" title="'.esc_attr__('Watch Video','sitesao').'" data-video-inline="'.esc_attr($video).'" href="#'.esc_attr($video_id).'" data-rel="magnific-popup-video"><i class="elegant_arrow_triangle-right_alt2"></i></a>';
			}
		}
				...
				<script type="text/javascript">
					...
					jQuery(document).on( 'click', '.product_cat_heding_upload', function( event ){
						...
						product_cat_heading_file_frame = wp.media.frames.downloadable_file = wp.media({
							title: '<?php echo esc_js(__( 'Choose an image', 'sitesao' )); ?>',
							button: {
								text: '<?php echo esc_js(__( 'Use image', 'sitesao' )); ?>',
							},
							multiple: false
						});
